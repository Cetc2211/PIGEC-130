...
export default function ScreeningManagement() {
    ...
    const handleGenerarMatriculas = async () => {
        ...
        console.log('[screening-management] Iniciando generación de matrículas...');

        for (const grupoId of selectedGroups) {
            const grupo = grupos.find(g => g.id === grupoId);
            if (!grupo) {
                console.warn(`[screening-management] Grupo no encontrado: ${grupoId}`);
                continue;
            }

            console.log(`[screening-management] Procesando grupo: ${grupo.nombre} (${grupoId})`);

            // Obtener estudiantes reales del grupo desde Firestore
            const resultadoEstudiantes = await obtenerEstudiantesGrupo(grupoId);
            console.log(`[screening-management] Estudiantes encontrados: ${resultadoEstudiantes.estudiantes.length}`);
            ...
            if (resultadoEstudiantes.success && resultadoEstudiantes.estudiantes.length > 0) {
                ...
                console.log('[screening-management] Usando estudiantes reales');
                ...
                console.log(`[screening-management] Usando estudiantes simulados (${grupo.totalEstudiantes})`);
                if (resultadoEstudiantes.error) {
                    console.warn(`[screening-management] Error al obtener estudiantes: ${resultadoEstudiantes.error}`);
                }

                estudiantesParaMatricular = Array.from({ length: grupo.totalEstudiantes || 1 }, (_, i) => ({
                    id: `est_${grupoId}_${i}`,
            ...
            console.log(`[screening-management] Llamando a generarMatriculasGrupo con ${estudiantesParaMatricular.length} estudiantes`);
            ...
            console.log(`[screening-management] Resultado: success=${resultado.success}, generados=${resultado.totalGenerados}, errores=${resultado.errores.length}`);

            todasMatriculas.push(...resultado.matriculas);
            todosErrores.push(...resultado.errores);
        }

        console.log(`[screening-management] Total matrículas generadas: ${todasMatriculas.length}`);
        console.log(`[screening-management] Total errores: ${todosErrores.length}`);

        setMatriculas(todasMatriculas);

        if (todosErrores.length > 0) {
            setErrorMatriculas(todosErrores.join('\n'));
            console.error('[screening-management] Errores:', todosErrores);
        }

        // Cargar lista del primer grupo para mostrar
        if (selectedGroups.length > 0) {
            console.log('[screening-management] Cargando matrículas guardadas desde Firestore...');
            const lista = await obtenerMatriculasGrupo(selectedGroups[0]);
            console.log(`[screening-management] Matrículas encontradas en Firestore: ${lista?.estudiantes.length || 0}`);
                        ...
                        <CardContent>
                            ...
                            {errorMatriculas && (
                                <Alert className="mb-4 border-red-200 bg-red-50">
                                    ...
                                    <AlertTitle className="text-red-800">Error al generar matrículas</AlertTitle>
