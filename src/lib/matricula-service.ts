...
export async function generarMatriculasGrupo(
  ...
  console.log(`[matricula-service] ========== GENERANDO MATRÍCULAS ==========`);
  console.log(`[matricula-service] Grupo: "${grupoNombre}" (ID: ${grupoId})`);
  console.log(`[matricula-service] Estudiantes recibidos: ${estudiantes.length}`);
  ...
  if (!db) {
    console.error('[matricula-service] ERROR: Base de datos no disponible (db es null)');
    result.errores.push('Base de datos no disponible');
    return result;
  }

  console.log('[matricula-service] ✓ Base de datos disponible');

  if (estudiantes.length === 0) {
    console.warn('[matricula-service] WARNING: No hay estudiantes para generar matrículas');
    result.errores.push('No hay estudiantes para generar matrículas');
    return result;
  }

  try {
    const batch = writeBatch(db);
    console.log('[matricula-service] ✓ Batch creado');

    const semestre = extraerSemestre(grupoNombre);
    let siguienteNumero = await obtenerSiguienteNumero(grupoId);
    console.log(`[matricula-service] Siguiente número de matrícula: ${siguienteNumero}`);
    ...
    for (const estudiante of estudiantes) {
      try {
        console.log(`[matricula-service] Procesando estudiante: ${estudiante.nombre}`);

        // Verificar si ya tiene matrícula
        const existente = await buscarMatriculaPorNombre(estudiante.nombre, grupoId);

        if (existente) {
          console.log(`[matricula-service]   → Matrícula existente encontrada: ${existente.matricula}`);
        ...
        console.log(`[matricula-service]   → Nueva matrícula generada: ${matricula}`);
        ...
        // Guardar en Firestore - filtrar valores undefined
        const docRef = doc(collection(db, COLECCION_MATRICULAS));
        const datosParaGuardar = {
          matricula: matriculaRegistro.matricula,
          nombreCompleto: matriculaRegistro.nombreCompleto,
          nombreNormalizado: matriculaRegistro.nombreNormalizado,
          grupoId: matriculaRegistro.grupoId,
          grupoNombre: matriculaRegistro.grupoNombre,
          semestre: matriculaRegistro.semestre,
          periodo: matriculaRegistro.periodo,
          fechaAsignacion: Timestamp.now(),
          evaluacionesCompletadas: matriculaRegistro.evaluacionesCompletadas,
          activo: matriculaRegistro.activo,
          // Solo incluir telefono y email si tienen valor
          ...(estudiante.telefono ? { telefono: estudiante.telefono } : {}),
          ...(estudiante.email ? { email: estudiante.email } : {})
        };
        batch.set(docRef, datosParaGuardar);
        ...
        console.error(`[matricula-service] ERROR con estudiante ${estudiante.nombre}:`, error);
        result.errores.push(`Error con estudiante ${estudiante.nombre}: ${error}`);
      }
    }

    console.log(`[matricula-service] Matrículas nuevas: ${matriculasNuevas}, existentes: ${matriculasExistentes}`);

    // Commit batch
    console.log('[matricula-service] Ejecutando batch.commit()...');
    await batch.commit();
    console.log('[matricula-service] ✓ Batch commit exitoso');

    result.success = true;
    console.log(`[matricula-service] ========== RESULTADO ==========`);
    console.log(`[matricula-service] Success: ${result.success}`);
    console.log(`[matricula-service] Total generados: ${result.totalGenerados}`);
    console.log(`[matricula-service] Total recuperados: ${result.totalRecuperados}`);
    console.log(`[matricula-service] Errores: ${result.errores.length}`);

  } catch (error) {
    console.error('[matricula-service] ERROR GENERAL en batch.commit():', error);
